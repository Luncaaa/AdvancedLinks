...
import me.lucaaa.advancedlinks.common.AdvancedLinks;import me.lucaaa.advancedlinks.common.data.MessageReceiver;import java.io.IOException;
...
public class UpdateManager {
    ...
    public void getVersion(final Consumer<String> consumer) {
        plugin.getTasksManager().runTaskAsynchronously(() -> {
            try {
    ...
    public void sendStatus(MessageReceiver console, String spigotVersion, String pluginVersion) {
        ...
        if (spigotVerMajor == pluginVerMajor && spigotVerMinor == pluginVerMinor) {
            messagesManager.sendColoredMessage(console, "&aThe plugin is up to date! &7(v" + pluginVersion + ")", true);

        } else if (spigotVerMajor > pluginVerMajor || (spigotVerMajor == pluginVerMajor && spigotVerMinor > pluginVerMinor)) {
            messagesManager.sendColoredMessage(console, "&6There's a new update available on Spigot! &c" + pluginVersion + " &7-> &a" + spigotVersion, true);
            messagesManager.sendColoredMessage(console, "&6Download it at &7https://www.spigotmc.org/resources/advancedlinks.117605/", true);

        } else {
            messagesManager.sendColoredMessage(console, "&6Your plugin version is newer than the Spigot version! &a" + pluginVersion + " &7-> &c" + spigotVersion, true);
            messagesManager.sendColoredMessage(console, "&6There may be bugs and/or untested features!", true);
        }
    }
}