package me.lucaaa.advancedlinks.v26_1.fabric;
...
public class FabricAdvancedLinks extends ModAdvancedLinks implements ModInitializer {
    ...
    public String replacePapiPlaceholders(String text) {
        if (supportsPapi()) {
            return Parsers.mm.serialize(asAdventure(Placeholders.SERVER_PLACEHOLDER_PARSER.parseComponent(TextNode.of(text), ServerPlaceholderContext.of(getServer()).asParserContext())));
        } else {
            return text;
        }
    }
    ...
    public ModLinkReceiver getLinkReceiver(ServerPlayer player) {
        return new ModLinkReceiver(this, player) {
            ...
            public String replacePapiPlaceholders(String text) {
                if (supportsPapi) {
                    return Parsers.mm.serialize(asAdventure(Placeholders.SERVER_PLACEHOLDER_PARSER.parseComponent(TextNode.of(text), ServerPlaceholderContext.of(player).asParserContext())));
                } else {
